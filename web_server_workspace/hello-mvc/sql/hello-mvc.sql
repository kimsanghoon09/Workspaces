...
create table attachment (
    ...
    constraints fk_attachment_board_no foreign key(board_no) references board(no) 
                                                            on delete cascade
);
...
insert into web.attachment(no, board_no, original_filename, renamed_filename)
values(seq_attachment_no.nextval, 59, 'test.txt', '20211007_233014432_12345.txt');
insert into web.attachment(no, board_no, original_filename, renamed_filename)
values(seq_attachment_no.nextval, 59, 'test.txt', '20211007_233014432_12345.txt');
...
--select b.*, (select count(*) from attachment where board_no = b.no) attach_cnt from board border by b.no desc
...
--select * from (select row_number() over (order by b.no desc) rnum, b.*, (select count(*) from attachment where board_no = b.no) attach_cnt from board b) where rnum between ? and ?
...
-- 최신 게시글
select * from board where no = 66;

select * from board_comment where board_no = 66;

-- 댓글 추가
insert into 
    board_comment
values( 
    seq_board_comment_no.nextval, 1, 'qwerty', '따뜻한 글 잘 읽었습니다.', 66, null, default
);
insert into 
    board_comment
values( 
    seq_board_comment_no.nextval, 1, 'sejong', '훈훈한 글 잘 읽었습니다.', 66, null, default
);
insert into 
    board_comment
values( 
    seq_board_comment_no.nextval, 1, 'admin', '주간 게시글로 선정되셨습니다. 축하드립니다.', 66, null, default
);

insert into 
    board_comment
values( 
    seq_board_comment_no.nextval, 2, 'admin', '주간 댓글로 선정되셨습니다. 축하드립니다.', 66, 1, default
);
insert into 
    board_comment
values( 
    seq_board_comment_no.nextval, 2, 'opqr', '저도 동감입니다 2222222', 66, 1, default
);
insert into 
    board_comment
values( 
    seq_board_comment_no.nextval, 2, 'opqr', '어! 훈민정음 아저씨다~', 66, 2, default
);
